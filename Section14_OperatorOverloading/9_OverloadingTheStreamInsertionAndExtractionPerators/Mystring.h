...
class Mystring{
    ...
    friend std::ostream &operator<<(std::ostream &os, const Mystring &rhs); //friend and provide the prototype. We've returning a std ostream reference. We'll do the insertion operator first. We've got a reference. The operator we're overloading is the insertion operator. And it's a binary operator. So we've got two parameters here. The first one is a std ostream. And we've got a mystringg, a const mystring reference. And we'll just call that right-hand side.
    friend std::istream &operator>>(std::istream &is, Mystring &rhs); // we're returning an input stream by reference again. And the operator is operator. And we'll be doing the extraction operator. And again, we've got two parameters the first one is the input stream by reference, and we'll just call that in. And we've got a my string object also by reference called it right-hand side. 
    //Notice how this one is not const because I do want to modify that object based on what I'm reading in from the input stream. Those are our two function prototypes.
